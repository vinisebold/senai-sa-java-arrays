    ...
    public static void main(String[] args) {
        ...
        for (int a = 0; a < qntdVendedores; a++) {
            if (somaVendas[a] >= 5000.0) {
                System.out.printf("%-16s | R$ %10.2f | %-13s | R$ %8.2f\n", nomesVendedores[a], somaVendas[a], "Exelente", somaVendas[a] * 1.1);
            } else if (somaVendas[a] > 3000.0 && somaVendas[a] < 4999.99) {
                System.out.printf("%-16s | R$ %10.2f | %-13s | R$ %8.2f\n", nomesVendedores[a], somaVendas[a], "Bom", somaVendas[a] * 1.05);
            } else {
                System.out.printf("%-16s | %10.2s | %-13s | %8.2s\n", nomesVendedores[a], somaVendas[a], "Regular", "R$ 0,00");
            }
        }
        ...
        int indiceMelhorVendedora = 0;

        for (int j = 1; j < qntdVendedores; j++) {
            if (somaVendas[j] > maiorVenda) {
                maiorVenda = somaVendas[j];
                indiceMelhorVendedora = j;
            }
        }

        System.out.println("\nMELHOR VENDEDORA DA SEMANA: " + nomesVendedores[indiceMelhorVendedora] + " (R$ " + maiorVenda + ")");
